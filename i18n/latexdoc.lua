...
---
-- @library Lua DocumentParser
-- The library parsers \ldots
--
...
-- This is a general list of options
-- that can be passed for output rendering
...
-- index. 
-- To document your program use the lua module latexluadoc. This is based
-- on a parser by ConTeXt for pgf and extended to add some fields for
-- latex.
...
--
-- @param type The type of the file. If omitted a |.lua| file is assumed.
-- @return the file
...
function DocumentParser.include(filename, typ)
  ...
  local file, error = io.open(filename)
  ...
  while i <= #lines do
    ...
    if lines[i]:match("^%-%-%-") then
      local infos = collect_infos (lines, i) -- collect the infos for following lines
                                             -- i.e after --- expect documentator items
...
-- Forwards:
local print_on_output, print_on_output_escape, print_docline_on_output, 
      open_mode, close_mode, print_lines_on_output
...
DocumentParser.addRenderer (
  ...
  function (infos, output)
    ...
    output[#output+1] =
      function ()
	if output[name] then
	  print("\\par\\emph{Alphabetical method summary:}\\par{\\small")
	  table.sort(output[name], function (a,b) return a.text < b.text end)
	  for _,l in ipairs(output[name]) do
	    print("\\texttt{\\hyperlink{" .. l.link .. "}{" .. l.text:gsub("_", "\\_") .. "}}\\par")
	  end
	  print("}")
	end
      end
  ...
  if l:match("^%s*@section") then
   ...
   elseif l:match("^%s*@param%s+") then
    ...
    print_on_output(output, "\\item[\\texttt{",
		    ...
		    "}] ",
		    l:match("%s@param%s+.-%s+(.*)"))
  elseif l:match("^%s*@author%s+") then
    if mode ~= "author" then
      close_mode (output, mode)
      mode = open_mode (output, "author")
      print_on_output(output, "", l:match("%s*@author%s*(.*)"))
    end     
...
local t = {["param"]   = {'\\begin{luaparameters}', '\\end{luaparameters}'},
             ["field"]   = {'\\begin{luaparameters}', '\\end{luaparameters}'},
             ["returns"] = {'\\begin{luareturns}', '\\end{luareturns}'},
             ["author"]  = {'\\begin{author}', '\\end{author}'}
             }

function open_mode (output, mode)
  
  if mode == t.mode then
    print_on_output(output, t.mode[1])
  end
  return mode
end

---
-- function  theis returns
-- closes the state and prints the line
-- @param output This is the description
-- @param mode  some description
-- @return mode 
-- 
...
function close_mode (output, mode)
  ...
  if mode == t.mode then
    print_lines_on_output(output, t.mode[2]) 
  end  
  return mode
end
...
---
-- @param infos Something required
--
