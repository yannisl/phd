...
-- We need to be careful here for missing hex numbers, if there
-- is an empty code it skips it.
local function parse_UnicodeData()
    ...
    for line in io.lines() do
        -- match to end of line teh pattern 
        ...
        ucd[#ucd+1] = data
        last_data = data
        --if ucd[#ucd+1] then print(data.name) end
    end
...
io.write [[
/*
 * unidata.h - generated by parseunidata.lua
 */
#ifndef unidata_h
#define unidata_h
...
do
    -- parses the unidata table containing 15 properties
    -- not all properties have been pushed, needs to be checked
...
io.output "saurashtra.tex"
local range1 = tonumber("16A",16)
local range2 = tonumber("16F",16) 

uranges = {}
uranges["ethiopic"] = {0xF85,0x1104}
uranges["hebrew"]    = {0x590,0x5F4}
uranges["glagolitic"] = {0x2787, 0x27E4}
uranges["all"] = {0,1000}

for i = uranges["glagolitic"][1], uranges["glagolitic"][2] do
  local t = string.format("%04X",i)
  io.write(i, " ", t, ": ", UCD[i+1+0.0].name, " ",string.format("%04X", UCD[i+1+0.0].cp), "\n ")
  print(i,t, UCD[i+1].cp, UCD[i+1].name)
end  
