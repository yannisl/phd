...
\usepackage{listings,expl3,xparse,l3sort}
\begin{document}
%%
...
\def\phd@bubblesort#1{%
    \ifx\@empty#1\else
        \lst@false
        \expandafter\phd@bubblesort@#1\relax,\relax,%
        \expandafter\phd@bubblesort@\expandafter,\lst@sorted
                                      \relax,\relax,%
        \let#1\lst@sorted
        \lst@if
            \def\lst@next{\phd@bubblesort#1}%
            \expandafter\expandafter\expandafter\lst@next
        \fi
    \fi}
    
\def\phd@bubblesort@#1,#2,{%
    \ifx\@empty#1\@empty
        \def\lst@sorted{#2,}%
        \def\lst@next{\phd@bubblesort@@}%
    \else
        \let\lst@sorted\@empty
        \def\lst@next{\phd@bubblesort@@#1,#2,}%
    \fi
    \lst@next}
...
\def\phd@bubblesort@@#1,#2,{%
    \ifx\relax#1\else
        \ifx\relax#2%
            ...
            \expandafter\expandafter\expandafter\phd@bubblesort@@@
        \else
...
\cs_set_eq:NN\addacronym  \clist_put_left:Nn 
   
   

\tt
\meaning\clist_put_left:Nn\\
\end{document}

 \addacronym\acronyms {AA}
\phd@bubblesort\acronyms\\
1  ~~\acronyms\\


\addacronym\acronyms {BA}
\phd@bubblesort\acronyms\\
\addacronym\acronyms {ZA}
\phd@bubblesort\acronyms\\
\meaning\acronyms\\
...
\end{document}
...
\ExplSyntaxOff
  ...
  The sorted numbers are \sorted{B,C, d, AA, CC, TT}.
\ExplSyntaxOff
\makeatother
