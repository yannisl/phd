...
\parskip3pt
 ...
 \begin{macro}{\nopagebreak} The command\cs{nopagebreak}\oarg{i} : \meta{i} = 0,...,4 attempts to stop the page breaking at this point, by inserting an appropriate amount of penalty. It also takes care to avoid introducing redundant spaces. It does not
...
As Frank Mittelbach noted\footnote{\url{http://tex.stackexchange.com/questions/83350/nopagebreak-and-penalty}}:
...
\begin{macro}{\\}         : initially defined to be |\newline|
...
Extra vertical space is added by the comand \cs{addvspace}\marg{skip}. It adds
...
\begin{macro}{\nobreakspace}
\begin{macro}{~}
The \cs{nobreakspace} command is a robust command that produces a horizontal
...
In the definition of |\pagebrak| and |\nopagebreak| observe the
use of |\@testopt|. This macro is defined in |ltdefns.dtx| and encapsulates
the most common call to |\@ifnextchar|. It takes two arguments. There is
also a robust version available \cs{protected@testopt}.
...
%  
% \begin{macro}{\@getpen}
The command |\@getpen| is a helper command to allocate penalties. This has
