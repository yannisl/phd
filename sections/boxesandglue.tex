...
\begin{multicols}{2}
\noindent \lettrine{L}{ike} their dimensions \TeX's boxes are not what one thinks when thinking of boxes. TeX's boxes come in basically two flavours, horizonatl boxes and vertical boxes. An \cs{hbox} is created by the command |\hbox{material}|. It has the following properties:
...
\begin{texexample}{}{}
     \fbox{%
          \vtop{\medskip
                    \hfill
                      \hbox{\includegraphics[width=1.5cm]{amato}}%
                    \hfill 
                   \medskip%
                }%
      }%
...
\begin{texexample}{}{}
   ...
   \fbox{%
          \vtop{\medskip
                    \hfill
                      \hbox{\includegraphics[width=1.5cm]{amato}}\kern10pt
                      \hbox{\includegraphics[width=1.5cm]{amato}}\kern10pt
                      \hbox{\includegraphics[width=1.5cm]{amato}}%    
                    \hfill 
                   \medskip%
...
\section{Boxes with \protect\LaTeXe}
...
from the following example is not an unqualified hbox it is an hbox preceded by leavevmode.\footnote{\url{http://tex.stackexchange.com/questions/105585/latex2e-makebox-hbox}}
...
\latex's analog of an unqualified \cs{hbox} is called \cs{mbox}. They are 
...
\setbox0=\hbox{T\kern-.1667em\lower.5ex\hbox{E}\kern-.125em X}
With the above we can just type \cs{box0} to get the \box0 !
...
The command \cs{resizebox}{width}{height}{object} can be used with tabular to specify the height and width of a table. The following example shows how to resize a table to 8cm width while maintaining the original width/height ratio.
...
called \cs{topskip} at the top of the column. The value
of this glue is derived from a complex formula involving 
the elements in the first line. We set\cs{topskip} to
the value of \cs{baselineskip} which simplifed matters considerably and did not produce any problems.
...
the top of the page.\cs{topskip} determines the amount of that glue, called
the \cs{topskip} glue \index{topskip glue}, by specifying what should be (provided that the
...
that the glue would be negative, TEX simply inserts no\cs{topskip} glue at
all on that page.


To understand better the effect of these rules, assume that\cs{topskip}
has no stretch or shrink and that the first item on the page is indeed a box.
Then if the height of that box is no greater than\cs{topskip}, its baseline
will be\cs{topskip} from the top of the page independently of its height.
On the other hand, if the height of the box is e greater than\cs{topskip},
its baseline will be \cs{topskip+e} from the top of the page. See pages 113--114 of The TEXbook for the remaining details of how\cs{topskip} works.
Plain \tex sets\cs{topskip} to 10pt.
