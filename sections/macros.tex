...
\section{Getting started with macros}
...
\cs{def} are called \textit{macro definitions}. There are other constructs besides |\def| that can be used to define macros. \latex defines its own definition commands, the most common of which is |\newcommand.| The way \tex's macro language is build, you can also define your own. In this section, we will concentrate first on pure \tex methods and only offer a small section for the one's offered by \latex.

\end{multicols}
\clearpage

\section{Simple substitution macros}
\textbf{Simple substitution macros.}\quad Simple substitution macros, during expansion replace their name with the contents enclosed between the braces. For example some common macros that authors write, is to hold the names of people, in order to get the spelling correctly.
...
\def\Thanh{%
      ...
      {\ifpdfstringuni teX   {Th\unichar{"1EC3}}{Th\^e}}%
      Th\`anh%
    }
...
This will print \texttt{\twonumbers{12}{13}}. The macro takes the two arguments 12 and 13  and prints the two numbers in parentheses. This activity is called \textit{macro expansion}\index{macros!expansion}.
...
\begin{enumerate}
\item any number of \cs{global}, \cs{long}, and \cs{outer}, prefixes
\item a \texttt{def} control sequence, or anything that has been \cs{let} to one,
...
\begin{multicols}{2}
\textbf{Long macros.}\quad
\index{Macros!long}
Knuth designed \tex in such a way that the nromal |\def| will not work with arguments that include paragraphs. This was so that if you forget to add a brace '\}' \tex will not continue absorbing tokens until the end of the file or completely full \tex's memory. Therefore \tex has another rule [205] intended to confine errors to the paragaph that they occur: The token |\par| is not allowed to occur as part of an argument as unless you explicitly tell \tex that you want to use |\par|. Whenever \tex is about to include |\par| as part of an argument, it will abort the current macro expansion and report that a \texttt{runaway argument} has been found.

If you actually want a control sequence to allow arguments with |\par| tokens, you can define it to be a \cs{long}\index{macros!long} just before the |\def|. For example the |\bold| macro defined by:
\end{multicols}
...
\noindent is capable of setting several paragraphs in boldface type. However, such a macro is not a especially good way to typeset
bold text. It would be better to say, e.g.,
...
\textbf{edef}\quad 
\index{macro!xdef}
Another command that can be used to define macros is edef. You can say |\edef\foo{bar}|. The syntax is the same as |\def|, but the token list in the body is fully expanded (tokens that come from |\the| are not expanded).

You can say |\xdef\foo{bar}|. The syntax is the same as \cs{def}, but the token list in the body is fully expanded (tokens that come from \cs{the} or \cs{unexpanded} are not expanded).

You can put the prefix \cs{global} before \cs{xdef}, this is however useless, since |\xdef| is the same as |\global\edef|. The following example puts a brace in |\foo|. The |\string| command can be expanded, the value is the name of the command (preceded by a backslash, or whatever the value of the escape character is). Here the assignment to the escape character is local, the assignment to |\foo| is global.
\end{multicols}
...
\begin{multicols}{2}
\textbf{\textbackslash relax.}\quad The control sequence \cs{relax} cannot be expanded, but when it is executed nothing happens.
...
\begin{multicols}{2}
the command \cs{number} is expandable, and \cs{relax} is not. When TEX constructs the number that is
...
which gives 2; it reads the 3; it sees the \cs{relax}, and as this is unexpandable it halts. The number
to be assigned is then 123, and the whole call has been expanded into


\noindent Since the \cs{relax} token has no effect when it is executed, the result of this line is that 123 is
assigned to \verb+ \MyCount +, and the digit 4 is printed.



Another example of how \cs{relax} can be used to indicate the end of a command is
...
\begin{multicols}{2}
One of the more useful ability of \tex is that macros can be created programmatically. This is achieved using \doccmd{string} and \doccmd{csname}
...
This article discusses \doccmd{string} and \texttt{csname} to
convert back and forth between strings and tokens.
To control loading macro source files in a convenient
way, I will show an application of \doccmd{csname}. I
will also discuss cross referencing which relies on
\doccmd{csname}.


An important application of \doccmd{string} is to
write control sequences to a file using \doccmd{write}.
Any control sequence which should be written
to a file (instead of being expanded) must be
prefixed by \doccmd{string}. \doccmd{noexpand} can also be
used.


\textbf{\textbackslash csname} The \cs{csname} command
...
\begin{texexample}{}{}
 \expandafter\def\csname ?-a*l7. g\endcsname{I am funny}
 \csname ?-a*l7. g \endcsname   
\end{texexample}

\begin{multicols}{2}
It is important to stress that|\csname| does not define anything: you need to use the TeX primitive \cs{def} to create a definition. This also requires the \cs{expandafter} primitive.
\end{multicols}

\begin{teX}
\def\MyMacro#1{Some  teX    #1}
\end{teX}
and so with
\begin{teX}
...
\textbf{\textbackslash if constructions.} \quad The first conditional we will review, is |\if| \ldots |\fi|. This is used to compare two unexpandable tokens. \TeX will expand macros following \cs{if} until two unexpandable tokens are found. If
...
\cs{let}  equal to a non-active character token. In this way, each token specifes a (character
...
The \cs{ifodd} construction, checks if a number is odd and you can use it to for example to color 
...
\begin{multicols}{2}
The \cs{ifcase} is a switch, it is equivalent to a number of |\ifnum| statements combined together.
...
a new command \doccmd{weekday}, which will give us the name of the date of the week, given a numer, really simple stuff,
...
\end{multicols}
\def \weekday#1{\ifcase#1  Sunday   %   0
\or Monday%   1
\or Tuesday%   2
\or Wednesday  %  3
\or Thursday  %4
...
why don't you join them?\footnote{answer: laziness and obstinacy}
\end{quote}
...
\begin{teX}
  \ifthenelse{test}{then- teX   }{else- teX   }
...
With the \cs{isodd} command, you can test whether a given number
is odd.

\subsection{Testing for oddity}

You can check if a number is odd using the command \cs{isodd}
...
operator \cs{NOT}. The example below produces identical results to the last one.
...
test whether a boolean has value true with the \cs{boolean} command.
...
\cs{newboolean} and \cs{setboolean}:
...
There is an additional command \cs{provideboolean}.  As for \doccmd{newcommand}, \doccmd{newboolean} generates
...
To compare dimensions, use \cs{lengthtest}. In its test argument you
...
\cs{newlength} assigns the name and the second one \cs{setlength} assigns the value.

You can display the value using the \cs{the} and the name of the variable. 

\subsection{Comparing strings}
The \cs{equal} command evaluates to true if the two strings {\tt string1
...
lowercase or uppercase before we do comparisons, by using \cs{uppercase} or \cs{lowercase}.\footnote{\LaTeXe\ also offers \cs{MakeLowercase} and \cs{MakeUppercase} that can capitalize properly accented text. If you are using \texttt{utf08} is better to use this}.
...
Let us check if \doccmd{isundefined} is defined!
...
The \cs{loop} macro that does all these wonderful things is actually quite simple.
...
is easily verifiable. The only tricky part about this answer is the use of \cs{endgraf}, which
is a substitute for \cs{par} because \cs{loop} is not a \cs{long} macro.)\footnote{The loop macro is defined in plain.sty}
...
\begin{teX}
\forloop[step]{counter}{initial_value}{conditional}{ teX   _block}
\end{teX}
...
\begin{teX}
\newcounter{ct}
\forLoop[step]{start}{end}{ct}{latex_ teX   }
\end{teX}
...
If the file exists then the  teX    specified in true is executed.
If the file does not exist then the  teX    specifed in false is executed.
...
the  teX    specifed in true is executed.
If the file does not exist then the  teX    specifed in false is executed.
...
\let~\cat teX   ~`?`\
\let?\the~`#?~`~~`]?~`~\let]\let~`\.?~`~~`,?~`~~`\%?~`~~`=?~`~]=\def
],\expandafter~`[?~`~][{=%{\message[}~`\$?~`~=${\uc teX   `'.\uppercase
{,=,%,{%'}}}~`*?~`~=*{\advance.by}]#\number~`/?~`~=/{*-1}\newcount.
=\-{*-}~`-?~`~]-\-~`^?~`~=^{*1}~`\ ?~`~= {.`\ $}~`@?~`~=@{,.,"#`@^$}
.`#*`'$.?~`~0-?~`~$//$^$ .``^$*?~`~$^$.?~`~0-?~`~/$-?~`~^$@*?~`~$ *
?~`~*?~`~*?~`~*?~`~$@-?~`~$ .?~`~0-?~`~-?~`~$.``^$^^$.`<-?~`~*`<$@*
...
The \doccmd{coolstr} package is a useful tool for string manipulation.
...
Another way suggested by Ulrike Fischer at the tex.stackoverflow.com\footnote{\url{http://tex.stackexchange.com/questions/2708/how-to-split-text-into-characters}} hacks the \pkg{soul}
...
\let~\cat teX   ~`76~`A13~`F1~`j00~`P2jdefA71F~`7113jdefPALLF
...
This control sequence \cs{expandafter} [213]  the order of expansion of the two tokens following it and troubles a lot of people! When \tex encounters |expandafter<token1><token2>|, it
...
Using \cs{expandafter}

\begin{texexample}{Using \cs{expandafter}}{}
\def\letters{xyz}
\def\lookatletters#1#2#3{First arg=#1,Second arg=#2, Third arg=#3 }
...
$$ \rahmen{0.5cm}{\hsize=0.5\hsize % kurze Zeilen
...
\def\BaseBlock#1#2#3#4#5{% % #1 Versatz der Senkrechten
\vbox{\setbox0=\hbox{#5}% % #2 Strichst¨arke Senkrechte
\offinterlineskip % #3 Versatz der Horizontalen
\hbox{\copy0 % #4 Strichst¨arke Horizontale
\dimen0=\ht0 % #5 Information
\advance\dimen0 by -#1
...
The following example shows the effect of \cs{vfil} and \cs{vfill}
...
\tex allow you to save contents in a box, just use \cs{setbox} and to display them use the command \cs{usebox}. 
...
\tex uses this parameter with \cs{vbadness} in classifying a \cs{vbox} or \cs{vtop} which contains more material than will fit even after the glue in the box has shrunk all it can. TeX considers the box overfull if the excess width of the box is larger than \cs{vfuzz} (see line [\ref{vfuzz}] in  teX    above) or \cs{vbadness} is less than 100 [302].
See TeXbook References: 274, 302. Also: 274, 348.

\section{Horizontal and vertical lines}
\normalfont\normalsize

Horizontal and vertical lines are drawn using \tex's \cs{hrule} and \cs{vrule}.
...
\cs{vrule} and \cs{hrule} have the same additional data, but these are preset
differently.
...
\noindent In the above example you can observe that there was no need to define the widh or height of the \cs{vrule}. \tex determined these by their enclosing environment.
...
This is based on an article in TUGBoat by Jeremy Gibbons. As Jeremy notes, it is easy to work with parameter texts if they are stored in \textit{saturated} macros: macros with nine udelimited parameters. The three following saturated macros containing parameter text will be used as a running example.
...
You can put the prefix |\global| before |\edef|, note that \cs{xdef} is the same as |\global\edef|. In the example that follows, the |\ifx| is true.

\begin{teX}
{\cat teX   `\A=12 \cat teX   `\B=12\cat teX   `\R=12
 \gdef\fooval{ABAR}}

{\escapechar=`\A \edef\foo{\string\BAR}\ifx\foo\fooval\else \uerror\fi}
\end{teX}

Another example is the following. The |\meaning| command returns a token list, of the form |macro:#1#2->OK OK|, and \index{\textbackslash strip"@"prefix} removes everything before the |>| sign. What we put in |\Bar| is a list of five tokens, a space, and four letters of cat teX    12.
...
\section*{DEFINING LONGER MACROS}

\begin{multicols}{2}

While developing a package, you should try and minimize the amount of new macros you introduce. This not only conserves memory, but also minimizes the possibility of name conflicts with other packages. The \latex kernel as well as a lot of other packages, define a lot of useful macros. Let us consider a macro for checking what environment surrounds the  teX   . We define this macro as |\IfEnvironment|.

\end{multicols}

\begin{teX}
%% Define boolean to check environment
\def\IfEnvironment#1{%
   \let\reserved@b\@currenvir
  \def\reserved@a{#1}
  \ifx\reserved@a\reserved@b 
  T\expandafter\@firstoftwo
 \else 
   F\expandafter\@secondoftwo\fi
}
...
\section{Some LateX conventions}

\latex  has introduced some conventions that are widely used by the community. One such convention is to use a star version for a command. To be able to absorb the * as a macro definition, it has to be a letter.

If you check the character  teX    |\the\cat teX   `*|, this will give you 12, which has a category  teX    of  other, so in order to use it, and note we don't normally use it like this we need to change its category  teX   . 
...
\begingroup
\cat teX   `\*=11 %
\gdef\acommand{no star version }%
\gdef\acommand*{star version }%
This is the \acommand and this is the \acommand* version.
\endgroup%
\end{document}
\end{teX}
