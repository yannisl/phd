...
%  \centering
\thispagestyle{plain}
{\centering  \includegraphics[width=0.7\linewidth]{./graphics/harnett}\par}
%  \caption{Rat digestion!}
%  \label{fig:rat}
%\end{marginfigure}
...
\item a \texttt{def} control sequence, or anything that has been \cmd{let} to one,
...
You can say |\xdef\foo{bar}|. The syntax is the same as \cmd{def}, but the token list in the body is fully expanded (tokens that come from \cmd{the} or \cmd{unexpanded} are not expanded).
...
\begin{multicols}{2}
\textbf{\textbackslash relax.}\quad The control sequence \cmd{relax} cannot be expanded, but when it is executed nothing happens.
...
\begin{multicols}{2}
the command \cmd{number} is expandable, and \cmd{relax} is not. When TEX constructs the number that is
...
\textbf{\textbackslash csname} The \cmd{csname} command
...
\begin{multicols}{2}
\Paragraph{\textbackslash ifcase.} The \cmd{ifcase} is a switch, it is equivalent to a number of |\ifnum| statements combined together.
...
Typing \texttt{\string\weekday\{12\}} will give you an error: 
 ...
 \weekday{12} \footnote{Not a real error, but we need to start thinking as to how to catch errors!}\footnote{\jobname, ~ \today }
...
lowercase or uppercase before we do comparisons, by using \cmd{uppercase} or \cmd{lowercase}.\footnote{\LaTeXe\ also offers \cmd{MakeLowercase} and \cmd{MakeUppercase} that can capitalize properly accented text. If you are using \texttt{utf08} is better to use this}.
...
%\ifthenelse{\isdefined{\isdefined}} 
%  {\string\isdefined\ is defined}
%  {\string\isdefined\ is not defined}
...
is a substitute for \cmd{par} because \cmd{loop} is not a \cmd{long} macro.)\footnote{The loop macro is defined in plain.sty}
...
%\newcounter{numbers}
%\forloop[2]{numbers}{5}{\value{numbers}<15}{\thenumbers ;}
...
Another way suggested by Ulrike Fischer at the tex.stackoverflow.com\footnote{\url{http://tex.stackexchange.com/questions/2708/how-to-split-text-into-characters}} hacks the \docpkg{soul}
