...
\chapter{The Art of Typesetting Mathematics}
\addtocimage{-12pt}{-30pt}{../images/tocblock-maths.jpg}
...
\medskip
   ...
   \end{leftcolumn}
   \begin{rightcolumn}
       \putimage[width=\linewidth]{./images/themathematician.jpg}
...
\begin{figure}
\centering
\includegraphics[scale=.35]{./images/halmos.png}
...
\noindent{\textbf{Do not start a sentence with an equation}}
\smallskip

This is an obvious rule, especially when indenting your paragraphs and it should be avoided at all costs.

\begin{description}

\item [Display math] In general mathematics typeset better when they are displayed. Use inline maths only for the simplest of equations and for explanations of symbols and the like. watch out for inconsistent spacing before and after displayed math.

\item [Correct badly sized math]
...
In fact, for the the sake of simplicity, the standard multiplication between letters, or between letters, or between a number and a letter, does not require any symbol. 

If, on the other hand, the multiplication is between two numbers, the $\times$ or $\cdot$ symbols are required to avoid ambiguity.
For example you should write

\[2\times 3=6 \text{ and not } 2\thickspace 3=6 \]


\item[Using the right font]

The Euler equation involves the five most important mathematical constants. First we typeset it with no space corrections\sidenote{\texttt{\textbackslash eu\^\,\{\textbackslash iu\textbackslash pi\}}},
...
A peculiar definition is required to properly
...
\section{Combining Maths with tikz}
...
It is interesting to note that Knuth believes that in equations this is unecessary. He is quoted in Typesetting Mathematics.
...
defines a macro \cs{A} that can be used both in and out of math mode to typeset a calligraphy script $\mathcal{A}$. 
...
\subsection{Writing Equations and tex}
...
If you get a too many math alphabets See egreg's solution at \footnote{\url{ http://tex.stackexchange.com/questions/100426/which-version-of-latex-permits-more-than-16-alphabets}} for extending them via LuaTeX or XeLaTeX.
