...
follows:
\begin{texexample}{Storing values in macros}{}
...
\begin{texexample}{Styles}{styles}
\pgfkeys{/par indent/.code={\parindent=#1}}
...
\begin{texexample}{The .initial handler}{initial}
...
\begin{handler}{{.store as glue}}
  This handler is defined by the \pkgname{phd} package for capturing glue or dimensions
  in a macro. 
\begin{texexample}{Custom handlers}{}
\pgfkeys{/my parindent/.store as glue=\myparindent}
\pgfkeys{/my parindent=10pt}
\parindent\myparindent
\lorem
\pgfkeys{/my parindent=20pt}
\parindent\myparindent
\lorem\par
\end{texexample}  
\end{handler}

The definition of an appropriate handler, can improve the user interface and also the programmer can benefit
by re-using code.
...
\begin{texexample}{Handling expansion}{pgfexpand}
\def\a{bottom}
\def\b{\a}
\def\c{\b}
