...
\label{ch:ltdefns}\label{kernel:ltdefns}
...
\begin{description}
\item [\refCmd{@namedef}]{\marg{name}}
  Expands to |\def\|\marg{name},
   except name can contain any characters.
 
\item[\refCmd{@nameuse}]{\marg{name}}
   Expands to |\|\meta{name}.

 
\item[\refCmd{@ifnextchar}]{X\Arg{Yes}\Arg{No}}
...
\item[\refCmd{@ifstar}]{\marg{Yes}\marg{No}}
...
\item[\refCmd{@ifdefinable}]\Arg{name}\marg{Yes}
       Executes \meta{Yes} if the user is allowed to define |\name|,
...
\item [\refCmd{newcommand}] |*|\marg{\cs{FOO}}\oarg{i}\marg{TEXT}\\
 ...
 \item [\refCmd{renewcommand} ] |*|\marg{\cs{FOO}}\oarg{i}\marg{TEXT}\\
  Same as |\newcommand|, except it checks if |\FOO| already defined.

 \item [\refCmd{newenvironment}] |*|\marg{FOO}\oarg{i}\marg{DEF1}\marg{DEF2}\\
         equivalent to:\\
         |\newcommand{\FOO}[i]{DEF1}| |\def{\endFOO}{DEF2}|\\
 (or the appropriate star forms).

 \item [\refCmd{renewenvironment}] Obvious companion to |\newenvironment|.

\end{description}

\subsection{\textbackslash @cons and \textbackslash @car}

The following macros are used extensively for lists.

\begin{description}
 \item [\refCmd{@cons}] : See description of |\output| routine.

 \item [{@car}] |\@car T1 T2 ... Tn\@nil| == |T1|  (unexpanded)

 \item [{@cdr}] |\@cdr T1 T2 ... Tn\@ni|l == |T2 ... Tn|     (unexpanded)

 \item [\refCmd{typeout}] \marg{message}\\ Produces a warning message on the terminal.

 \item [\refCmd{typein}]\marg{message}\\
 ...
 \DescribeMacro
  ...
  Same as above, except defines |\CS| to be the input
                      instead of executing it.\footnotechanges{LaTeX209}{1992/03/18}
...
% define \@elt to be a space
\def\@elt{\space}

% type the meaning
\meaning\mylist

% define \@elt to print its argument with a comma
\def\@elt#1{#1, }
...
% \footnotechanges{v1.2u}{1995/10/16}{(DPC) Use \cs{@testopt} /1911}
\begin{teX}
\def\new@command#1{%
...
% \footnotechanges{LaTeX2e}{1993/11/23}{Macro added}
\end{docCommand}
...
\end{docCommand}
% \footnotechanges{LaTeX2e}{1993/11/23}{Macro interface changed}
% \footnotechanges{v1.1g}{2004/01/23}{Use kernel version of
...
\end{teX}
%
% \footnotechanges{v1.2q}{1995/10/02}
...
% \footnotechanges{v1.2z2}{1998/03/04}
...
%  \footnotechanges{LaTeX2e}{1993/12/20}
%                {Kept old version of \cs{@reargdef}, for array.sty}
% \footnotechanges{v1.0b}{1994/03/12}{New defn, in terms of \cs{@yargdef}}
%  \footnotechanges{v1.2y}{1996/07/26}{third arg picked up by \cs{@yargdef}}
\begin{teX}
...
\end{docCommand}
%    \emph{a--e}.)
% \footnotechanges{LaTeX2e}{1993/11/23}{Macro reimplemented and extended}
% \footnotechanges{v1.1f}{1994/05/2}{Removed surplus \cs{space} in error}
\begin{teX}
...
\end{docCommand}
%  \footnotechanges{v1.2y}{1996/07/26}{use \cs{relax} in place of empty arg}
%  \footnotechanges{v1.2y}{1996/07/26}{use \cs{noexpand} instead of \cs{string}}
% \footnotechanges{v1.2z1}{1997/10/21}{Use \cs{begingroup}/\cs{endgroup} rather
...
\end{docCommand}
 ...
 known trick to get the \cmd{\string} not to print the backslash character, by setting the |\escapechar|
 to -1. This is really an esoteric trick.\index{hacks>escapechar}
...
\begin{teX}[gobble=0,numbers=none]
\def\@if#1#2{% 
...
\subsection{Other argument grabbers}
...
\end{docCommand}
%
% \footnotechanges{v1.2e}{1994/11/04}{Rewrote protected short commands
...
\end{teX}
% \footnotechanges{v1.2s}{1995/10/06}
%     {Add \cs{aut@global} in autoload version}
\begin{teX}
%<autoload>\aut@global
   \edef#1{%
...
\end{docCommand}
%
% \footnotechanges{v1.2e}{1994/11/04}{Added commands for setting and restoring
%    \cs{protect}.  ASAJ.}
%
...
%    displaying.
% \footnotechanges{v1.2o}{1995/07/03}{Use \cs{@typeset@protect} for init}
\begin{teX}
...
% \footnotechanges{v1.2j}{1995/04/29}{Init \cs{protect} here}
\begin{teX}
\set@typeset@protect
\end{teX}
\end{docCommand}
...
% \footnotechanges{LaTeX2e}{1993/11/23}{Redefined to remove a trailing \cs{fi}}
...
\begin{docCommand}{@onelevel@sanitize}{}
    ...
    done when they are defined.\footnotechanges{v1.2c}{1994/10/30}{Macro added}
