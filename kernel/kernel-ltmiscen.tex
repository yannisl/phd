...
\cxset{fashion image=fashion-04.jpg,
       palette budapest}
...
The \cs{begin} command takes one argument, checks if the argument is 
defined as a command and provides code to remember the line number. 
It also captures the environment name.
...
\begin{texexample}{Environment examples}{ex:begin}
\newenvironment{mytest}{}{}
\begin{mytest}
\makeatletter
% Typesets mytest
Environment name: \@currenvir\\ 
% Typesets on input line 7
Environment line: \on@line 

\ExplSyntaxOn
\cs_generate_variant:Nn \str_if_eq:nnTF {xnTF}
\str_if_eq:xnTF {\@currenvir}{mytest}{\PASS}{\FAIL}
\ExplSyntaxOff
\makeatother
\end{mytest}
\end{texexample}
...
\begin{macro}{\end}
The definition of |\end| is straight forward. It defines
the end command for the environment such as |\endmyenvironment|.
Checks that we ar ein the same environment, if not will
issue @badend error.
% \changes{v1.1e}{1996/07/26}{remove \cs{global} before \cs{@ignore...}}
\begin{teX}
\def\end#1{%
...
\footnotechanges{v1.0q}{1994/05/24}{Use \cs{@empty} as outer default}
\begin{teX}
...
\section{Center, Flushright, Flushleft}
